...
class MoviesController extends Controller
{
    ...
    public function movies_watch($slug,$id)
    {
        ...
        if($movies_info->video_access=="Paid")
        {
            if(Auth::check())
            {
                if(Auth::User()->usertype =="User")
                {
                    ...
                    {
                        return redirect('membership_plan')->with('error_flash_message', 'Please subscribe to a plan to access this video.');
                    }
                    if($user_info->plan_payment_status==0)
                    {
                        return redirect('dashboard')->with('error_flash_message', 'Your Subscription is on Hold please contact to Admin');
                    }
                }
            }
            else
        ...
        // Check if the view is already recorded in the session
        if (!session()->has("viewed_$v_id")) {
        ...
        if($movies_info->upcoming==1)
        {
            ...
            return back()->with('error_flash_message', 'This movie is upcoming, please wait for the release date.');
        }
        else
        {
